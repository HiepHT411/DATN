...
public class PartController {
    ...
    public String updatePart(@PathVariable("id") long id, @Valid Part part,
        ...
        if (Objects.nonNull(part.getPartNumber()) && part.getPartNumber() > 0) {
            partToUpDate.setPartNumber(part.getPartNumber());
        }
        if (Objects.nonNull(part.getPartType()) && StringUtils.isNotBlank(part.getPartType().toString())) {
            partToUpDate.setPartType(part.getPartType());
        }
        if (Objects.nonNull(part.getDescription()) && StringUtils.isNotBlank(part.getDescription())) {
            partToUpDate.setDescription(part.getDescription());
        }
        if (Objects.nonNull(part.getTimes())) {
